...
Python wrapper for Google's [Mediapipe Multi-Hand Tracking](https://github.com/google/mediapipe/blob/master/mediapipe/docs/multi_hand_tracking_mobile_gpu.md) pipeline. Does not implement the part of the pipeline where `NormalizedRectVectorHasMinSize` that would prevent `MultiHandDetection` from running on some frames. Yet.
